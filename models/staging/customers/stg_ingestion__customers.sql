...
with source as (
    ...
    select * from {{ source('raw_ingestion__tables', 'customers') }}
...
standardized as (
    ...
    select
        {{ generate_surrogate_key(["customer_id"]) }} as id,
        {{ natural_key_as_string("customer_id") }} as nk,

        {{ std_string("customer_name") }} as name,
        {{ std_string("customer_email") }} as email,
